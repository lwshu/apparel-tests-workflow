# -*- coding: utf-8 -*-
"""
Created on Fri Apr 01 14:05:47 2016
...
paths=[]
wd = "C:/Users/lshu0/Documents/Teradata driver/kmart"  # change to directory of all excel files
pattern   = "*.xls"
for x in os.walk(wd):
    paths.extend(glob.glob(os.path.join(x[0],pattern))) 

metrics = ['Price', 'Margin', 'Sales', 'Units', 'ASP']
...
output=output.T
metric_list= ['Week','Format','Test Name','# of test stores','# of all stores','% stores touched','Total Revenue in Test Stores','ASP Test Expected','ASP Test Actual','ASP % Lift','ASP $ Lift','Units Exp/st','Units Act/st','Units Test Expected','Units Test Actual','Units % Lift','Units Incremental','Sales Exp/st','Sales Act/st','Sales Test Expected','Sales Test Actual','Sales % Lift','Sales Incremental','Margin Exp/st','Margin Act/st','Margin Test Expected','Margin Test Actual','Margin % Lift','Margin Incremental']
output=output.reindex(columns=metric_list)
output.to_excel('kmart_output.xlsx',index= False)